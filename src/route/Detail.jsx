...
function DetailPage({ write }) {
	...
	return (
		<div className='window'>
			...
			{modal === true ? <DeleteCheckModal onClose={() => SetModal(false)} navigate={navigate} /> : null}
		</div>
	);
}
...
function CommentSection() {
	...
	return (
		<div style={{ marginTop: '20px' }}>
			...
			<div className='window'>
				<div className='title-bar'>
					<div className='title-bar-text'>📝 댓글 목록 ({comments.length}개)</div>
					<div className='title-bar-controls'>
						<button aria-label='Minimize'></button>
						<button aria-label='Maximize'></button>
						<button aria-label='Close'></button>
						...
						<div key={comment.id} style={{ marginBottom: '15px' }}>
							<fieldset>
								...
								<div
									...
									<div>
										<span style={{ fontWeight: 'bold', color: '#000080' }}>👤 {comment.author}</span>
										<span
											style={{
												marginLeft: '10px',
												fontSize: '12px',
									...
									<div style={{ display: 'flex', gap: '5px' }}>
										<button style={{ fontSize: '11px', padding: '2px 8px' }}>✏️ 수정</button>
										<button style={{ fontSize: '11px', padding: '2px 8px' }}>🗑️ 삭제</button>
									</div>
								</div>
				...
				<div className='window-body'>
					<fieldset>
						...
						<div className='field-row'>
							<label>작성자:</label>
							<input type='text' placeholder='닉네임을 입력하세요' style={{ width: '150px', marginRight: '10px' }} />
						</div>
						<div className='field-row' style={{ marginTop: '10px' }}>
							<label>내용:</label>
						</div>
						<textarea rows='4' style={{ width: '100%', resize: 'none' }} placeholder='댓글을 입력하세요...' value={newComment} onChange={(e) => setNewComment(e.target.value)} />
